 ...
 *
 */
public class WAT184 extends TestBase {
	...
	/**
	 * Method for Verify Author ORCiD as displayed as URI
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	public void validateCombinedAuthorORCiD(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,"Verify that System displays the author's ORCiD (when available) as a URI");
			pf.getAuthorRecordPage(ob).orcidAsURI();
			test.log(LogStatus.PASS, "Author's ORCiD displayed as a URI");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Author's ORCiD not displayed as a URI","Author_orcid_URI_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
