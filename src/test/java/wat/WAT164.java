...
/**
 * Class for Verify that System displays the author's ORCiD (when available) as a URI ||
 * Verify that upon clicking the orcid link user shall be taken to the ORCiD record in a new browser window/tab
 * 
 * @author UC202376
 *
 */
public class WAT164 extends TestBase {
	...
	/**
	 * Method for Verify Combined Author RID Functionality
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	@Parameters({"lastName"})
	public void validateCombinedAuthorORCiD(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Verify author's ORCiD (when available) as a URI and upon clicking the orcid link user shall be taken to the ORCID record in a new browser window/tab");
			pf.getAuthorRecordPage(ob).orcid(test);
			test.log(LogStatus.PASS, "ORCiD URI is displayed and upon clicking user shall be taken to the new window");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Combined Author ORCiD URI not displayed and not taken to new window ","RID_Fail_for_Combined_Author");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
