 ...
 * @author UC202376
 */
public class WAT153 extends TestBase {
	...
	/**
	 * Method for validate Ballon Popover outside click in Author Search page
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	@Test(dependsOnMethods = {"testLoginWATApp"})
	@Parameters({"header","content"})
	public void validateBallonPopoverOutsideClick(String header,String content) throws Exception {

		try {
			test.log(LogStatus.INFO, "Validate Ballon Popover Header and Content");
			pf.getSearchAuthClusterPage(ob).ballonPopoverContent(header, content);
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Ballon Popover Header and content  not giving as expected",
					"Ballon_popover_header_and_content_Fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
