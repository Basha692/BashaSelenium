...
/**
 * Class for Verify that System must display a maximum of 5 organization
 * @author UC202376
 *
 */
public class WAT179 extends TestBase {
	...
	/**
	 * Method for Verify Combined Author organization tab max 5 organizatoins
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	@Test(dependsOnMethods = {"testLoginWATApp"})
	@Parameters({"lastName"})
	public void validateCombinedAuthorOrgCount(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,"Verify Combined author display a maximum of 5 organizations");
			...
			test.log(LogStatus.PASS, "Combined author displayed a maximum of 5 organizations");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Combined author not dispalyed a maximum of 5 organizations","org_tab_combined_author_max_5_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
