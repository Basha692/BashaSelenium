...
/**
 * Class for Verify that System must display a 'Submit updates' button on the combined author record page.
 * @author UC202376
 */
public class WAT187 extends TestBase {
	...
	@Parameters("lastName")
	public void combinedAuthorSubmitUpdates(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO, "Verify Submit updates button display on the combined author record page.");
			pf.getAuthorRecordPage(ob).checkSubmitUpdateBtn();
			test.log(LogStatus.PASS, "Submit updates button displayed on the combined author record page.");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Submit updates button not displayed on the combined author record page", "submit_update_button_fail_combined_author_record");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
