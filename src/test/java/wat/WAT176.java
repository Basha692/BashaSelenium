...
/**
 * Class for Verify that each publication returns to original state by clicking on ‘Less’ link
 * @author UC202376
 *
 */
public class WAT176 extends TestBase {
	...
	/**
	 * Method for Validate Publications authors less to more
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	public void validatePublicationsAuthrosLessToMoreLink(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Each publication returns to original state by clicking on ‘Less’ link");
			pf.getAuthorRecordPage(ob).publicationsAuthorsLessLink();
			test.log(LogStatus.PASS, "Each publication returned to original state by clicking on ‘Less’ link");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Each publication not returned to original state by click on Less Link","each_publication_less_to_more_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
