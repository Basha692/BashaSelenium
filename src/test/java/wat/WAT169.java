...
/**
 * Class forVerify that author records list in the author results page should support following sorted options: 
 * 1.Relevance (Top hits, aggregated by count {Number of publications}), 
 * 2.Publication years (newest first) 
 * 3. Publication years (oldest first).
 * 
 * @author UC202376
 *
 */
public class WAT169 extends TestBase {
	...
	@Parameters("dropdownOptions")
	public void sortedByDropdownOptions(String dropdownOptions) throws Exception {

		try {
			test.log(LogStatus.INFO, "valiate author records list in the author results page should support sorted options");
			pf.getSearchAuthClusterResultsPage(ob).sortedByDropdownOptions(dropdownOptions,test);
			test.log(LogStatus.PASS, "author records list in the author results page should supported sorted options");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Author records list in the author results page should not supported for sorted options","sorted_dropdown_options_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
