...
/**
 * Class for Verify that user is able to sort author records/results using Sort by 'Publication years (newest first)'
 * 
 * @author UC202376
 *
 */
public class WAT167 extends TestBase {
	...
	/**
	 * Method for verify Search results dropdown in Publication years (newest first)
	 * 
	 * @throws Exception, When Something unexpected
	 */
	@Test(dependsOnMethods = {"searchAuthorCluster"})
	public void sortedByPublicationsNewestFirstDropdown() throws Exception {

		try {
			test.log(LogStatus.INFO, "Validate Author search results in sort order dropdown Publication years (newest first)");
			test.log(LogStatus.INFO, "Perform Search results dropdown using Publication years (newest first)");
			pf.getSearchAuthClusterResultsPage(ob).sortedByDropddown("Publication years (newest first)", test);
			pf.getSearchAuthClusterResultsPage(ob).sortedByPublicationYearsNewestFirst(test);
			test.log(LogStatus.PASS, "All Author search results in sort order i.e. Publication years (newest first) displayed correctly");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Author search results are not in  sort order Publication years (newest first)","sorted_by_pub_year_newest_first_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
