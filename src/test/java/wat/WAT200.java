...
/**
 * Class Verify that publication count should decrease when user removes publication for single author	
 * @author UC202376
 */
public class WAT200 extends TestBase {
	...
	public void removePublicationSingleAuthor(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO, "Verify that publication count should decrease when user removes publication for single author");
			pf.getAuthorRecordPage(ob).validatePubRemoveCount();
			test.log(LogStatus.PASS, "Publication count decreased  when user removes publication for single author");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Publication count not decreased  when user removes publication for single author", "publicaton_count_not_decreased_when_remove");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
