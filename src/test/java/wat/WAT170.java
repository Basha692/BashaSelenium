...
/**
 * Class for Verify that the system should only recommend papers to an author record where a last name on the paper
 * matches a last name on the author record.
 * @author UC202376
 *
 */
public class WAT170 extends TestBase {
	...
	public void validateRecommendPaperLastname(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Verify system should only recommend papers to an author record where a last name on the paper matches a last name on the author record");
			
			pf.getAuthorRecordPage(ob).recommendPapersLastName(lastName);
			test.log(LogStatus.PASS, "All Recommend Papers last name matched with last name on the author Record");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Recommend Papers last name not matched with last name on the author Record","Recommend_papers_last_name_not_matched_author_record_last_name");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
