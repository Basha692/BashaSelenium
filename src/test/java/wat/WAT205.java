 ...
 * @author UC202376
 */
public class WAT205 extends TestBase {
	...
	@Parameters("lastName")
	public void publicationMetadataTitleAuthor(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO, "Verify Publication metadata details such as publicaiton Title and Authors");
			pf.getAuthorRecordPage(ob).publicationMetaDataTitleAndAuthors();
			test.log(LogStatus.PASS, "Publicaiton metadata details Title and Authors availble in Author record page");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Publicaiton metadata details Title and Authors not availble in Author record page", "publication_metadata_details_not_avalable");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
