 ...
 *
 */
public class WAT24 extends TestBase {
	...
	public void testSearchAuthorCluster(String lastName,String firstName,
			String countryName,
		...
		try {
			...
			logFailureDetails(test, t, "Author Search Fail",
					"author_search_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
	...
	public void validateSearchResultsCount() throws Exception {

		try {
			test.log(LogStatus.INFO, "Validate Author Search Results count with Publications count");
			pf.getSearchAuthClusterResultsPage(ob).matchSearchResultsCountWithPublicationsCount(test);
			test.log(LogStatus.PASS, "Author Search Results count Matches with Publications count");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Author Search Results count not matching with Publications count",
					"author_search_results_count_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
