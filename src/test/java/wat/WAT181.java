...
/**
 * Class for Verify that Alternatives name tab getting highlighted when user click on it for combine 2 or more author records
 *  @author UC202376
 */
public class WAT181 extends TestBase {
	...
	/**
	 * Method for validate alternative names tab highlighted by highlight bar in Author Record page for combine author
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	public void authorRecordPageAlternativeNameTab(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO, "Verify Alternative names tab getting highlight by highlight bar when user clicked Alternative names tab for combined author");
			pf.getAuthorRecordPage(ob).checkForAlternativeNames();
			pf.getAuthorRecordPage(ob).clickAlternativeNamesTab();
			test.log(LogStatus.PASS, "Alternative names tab highlighted by highlight bar when user clicked Alternative names tab for combined author");
			pf.getWatPageInstance(ob).logoutWAT();
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Alternative names tab not highlighted by highlight bar when user clicked Alternative names tab for combined author",
					"combined_author_alternative_names_tab_not_highlighted");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
