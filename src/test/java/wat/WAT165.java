...
/**
 * Class for Verify that System displays the author's ResearcherID (when available)	||
 * Verify that upon clicking the Researcher ID link user shall be taken to the ResearcherID record in a new browser window/tab

 * @author UC202376
 *
 */
public class WAT165 extends TestBase {
	...
	@Parameters({"lastName"})
	public void validateCombinedAuthorRID(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Verify author's RID (when available) as a URI and upon clicking the orcid link user shall be taken to the RID record in a new browser window/tab");
			pf.getAuthorRecordPage(ob).rid(test);
			test.log(LogStatus.PASS, "RID URI is displayed and upon clicking user shall be taken to the new window");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Combined Author RID URI not displayed and not taken to new window ","RID_Fail_for_Combined_Author");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
