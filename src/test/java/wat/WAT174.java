...
/**
 * Class for Verify that each publication list should display first few authors followed by ‘More’ link
 * @author UC202376
 *
 */
public class WAT174 extends TestBase {
	...
	public void validatePublicationsAuthrosMoreLink(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Each publication list should display first few authors followed by ‘More’ link");
			pf.getAuthorRecordPage(ob).publicationsAuthorsMoreLink();
			test.log(LogStatus.PASS, "Each publication list should displayed first few authors followed by ‘More’ link");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Each publication list not displayed first few authors followed by ‘More’ link","each_publication_author_more_link_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
