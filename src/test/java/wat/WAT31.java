...
/**
 * Class for Verify load more author search results using page scroll down if search results are
 * morethan 10
 * 
 * @author UC202376
 *
 */
public class WAT31 extends TestBase {
	...
	public void defaultSortOrder() throws Exception {
		...
		try {
			test.log(LogStatus.INFO, "Validate Author search results in default sort order (sorted by relevance) ");
			pf.getSearchAuthClusterResultsPage(ob).sortByRelevance(test);
			test.log(LogStatus.PASS, "All Author search results in default sort order (sorted by relevance) ");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Author search results are not in default sort order (sorted by relevance) ","sorted_by_relevace_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
