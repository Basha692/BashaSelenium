...
/**
 * Class for Verify that A paper should only be recommended if at least one of its last names exactly matches one of the
 * last name variants on the author record.
 * @author UC202376
 *
 */
public class WAT172 extends TestBase {
	...
	public void validateRecommendPaperLastname(String lastName,String authorName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,
					"Verify that A paper should only be recommended if at least one of its last names exactly matches one of the last name variants on the author record.");
			pf.getAuthorRecordPage(ob).recommendPapersLastnameCount();
			test.log(LogStatus.PASS, "All Recommend Papers last name components matches atleast one of author record last names");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "All Recommend Papers last name components not matches atleast one of author record last names","Recommend_papers_last_name_not_matched_atleast_one_author_record_last_name");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
