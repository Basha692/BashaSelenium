 ...
 * @author UC202376
 */
public class WAT152 extends TestBase {
	...
	public void validateBallonPopoverOutsideClick() throws Exception {

		try {
			test.log(LogStatus.INFO, "Validate Ballon Popover Close Behaviour using outside click");
			pf.getSearchAuthClusterPage(ob).ballonPopoverOutsideClose();
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Ballon Popover Close outside click not working as expected",
					"Ballon_popover_Fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
