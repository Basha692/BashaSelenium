...
/**
 * Class for Verify that system must submit the feedback when user clicks on 'Submit updates' button ||
 * Verify that success confirmation message relates to submit feedback should display when user clicks on 'Submit updates' button
 * @author UC202376
 */
public class WAT189 extends TestBase {
	...
	/**
	 * Method for validate Submit updates modal content
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	@Parameters("lastName")
	public void combinedAuthorSubmitAuthorshipFeedback(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO, "System submit the feedback wher user clicks on submit button and success toast notification should display");
			pf.getAuthorRecordPage(ob).verifySubmitUpdatesModal();
			pf.getAuthorRecordPage(ob).submitAuthorship();
			test.log(LogStatus.PASS, "System submitted the feedback successfully wher user clicks on submit button and success toast notification displayed");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "System not submitted the feedback  wher user clicks on submit button and no success toast notification displayed", "submit_update_feedback_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
