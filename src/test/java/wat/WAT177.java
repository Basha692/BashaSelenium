...
/**
 * Class Verify that System must display an organizations tab after combine 2 or more records
 * @author UC202376
 *
 */
public class WAT177 extends TestBase {
	...
	/**
	 * Method for Verify Combined Author organization tab
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	@Test(dependsOnMethods = {"testLoginWATApp"})
	@Parameters({"lastName"})
	public void validateCombinedAuthorOrgTab(String lastName) throws Exception {
		...
		try {
			...
			test.log(LogStatus.INFO,"Verify Display an organizations tab after combine 2 or more records");
			pf.getAuthorRecordPage(ob).checkOrganizationsTab();
			test.log(LogStatus.PASS, "Organizations tab Displayed after combined 2 or more records");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Organizations tab not after combined 2 or more records","org_tab_fail_combined_two_records");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
