...
/**
 * Class to test profile default avatar displayed in each Author cluster publication
 * 
 * @author UC202376
 *
 */
public class WAT25 extends TestBase {
	...
	/**
	 * Method for Search Author cluster
	 * 
	 * @param lastName
	 * @throws Exception, When Something unexpected
	 */
	...
	@Parameters({"lastName"})
	public void testSearchAuthorCluster(String lastName) throws Exception {
	...
	/**
	 * Method for Validate Default Avatar displayed each publication cart
	 * @throws Exception
	 */
	@Test(dependsOnMethods = {"testSearchAuthorCluster"})
	public void validateDefaultAvatarEachPublication() throws Exception {

		try {
			test.log(LogStatus.INFO, "Validate Default Avatar displayed for each publication cart");
			pf.getSearchAuthClusterResultsPage(ob).defaultAvatarEachPublication(test);
			test.log(LogStatus.PASS, "Default Avatar Available for each publication cart");
			pf.getBrowserActionInstance(ob).closeBrowser();
		} catch (Throwable t) {
			logFailureDetails(test, t, "Default Avatar not displayed for each publication cart",
					"default_avatar_each_publication_fail");
			pf.getBrowserActionInstance(ob).closeBrowser();
		}
