...
/**
 * The RCC113 program covers Add Article/Patent/Post to Multiple Groups from Record view Page
 * @author Chinna
 * 
 */
public class RCC115 extends TestBase {
	...
	@Parameters("docTitle") 
	public void addDocumentToMultipleGroupsFromSearchResultsPage(String docTitle) throws Exception {

		test.log(LogStatus.INFO, "Add Article/Patent/Post into the group from Record view page");
		try {
			ob.navigate().refresh();
			test.log(LogStatus.INFO, "Search for Article/Patent/Post");
			pf.getSearchProfilePageInstance(ob).enterSearchKeyAndClick(docTitle);
			test.log(LogStatus.INFO, "Add Article into Multiple Groups from Record View page");
			String articleTitle=pf.getSearchResultsPageInstance(ob).getArticleTitle();
			pf.getSearchResultsPageInstance(ob).clickOnArticleOrPatentTitle(articleTitle);
			pf.getpostRVPageInstance(ob).validateDocumentInRecordViewPage(articleTitle);
			pf.getSearchResultsPageInstance(ob).addDocumentToMultipleGroup(Arrays.asList(groupTitle,groupTitle2));
			
			/*test.log(LogStatus.INFO, "Add Patent into Multiple Groups from Record View page");
			pf.getSearchProfilePageInstance(ob).enterSearchKeyAndClick(docTitle);
			...
			test.log(LogStatus.INFO, "Add Post into Multilple Groups from Record View page");
			pf.getSearchProfilePageInstance(ob).enterSearchKeyAndClick(docTitle);
