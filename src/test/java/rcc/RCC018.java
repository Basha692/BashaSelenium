...
public class RCC018 extends TestBase {
	...
	public void testGroupCreation() throws Exception {
		...
		try {
			...
			maximizeWindow();
			ob.navigate().to(host);
			loginAs("RCCTESTUSER056", "RCCTESTUSERPWD056");
			...
			pf.getGroupDetailsPage(ob).inviteMembers(LOGIN.getProperty("RCCPROFILE57"));
			...
			maximizeWindow();
			ob.navigate().to(host);
			loginAs("RCCTESTUSER057", "RCCTESTUSERPWD057");
			...
			try {
				Assert.assertTrue(pf.getGroupDetailsPage(ob).verifyConfirmationModalContents(modalLabel, modalInfoText,
						"Remove"));
				test.log(LogStatus.PASS, "Post remove confirmation modal validation success");

			} catch (Throwable t) {
				logFailureDetails(test, t, "Post remove confirmation modal validation Failed",
						"_post_remove_confirmation_validation_failed");
			}
			...
			try {
				...
				logFailureDetails(test, t, "Post is removed when user submits remove post action",
						"Group_remove_close_failed");
			}
			...
			try {
				...
				test.log(LogStatus.PASS,
						"No records informational text is displayed correcty in Post tab when there are no posts");
			} catch (Throwable t) {
				logFailureDetails(test, t,
						"No records informational text is not displayed correcty in Post tab when there are no posts",
			...
			recordTitle = pf.getSearchResultsPageInstance(ob).getPostsTitle();
			pf.getSearchResultsPageInstance(ob).addDocumentToGroup(title);
			waitForAjax(ob);
			pf.getLoginTRInstance(ob).logOutApp();
			closeBrowser();
			...
			try {
				Assert.assertTrue(pf.getGroupDetailsPage(ob).getPostsCounts() == 0);
				test.log(LogStatus.PASS, "Post count is decreased when user submits remove post action");

			} catch (Throwable t) {
				logFailureDetails(test, t, "Post is not removed when user submits remove post action",
