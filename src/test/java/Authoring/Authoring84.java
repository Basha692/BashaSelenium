...
public class Authoring84 extends TestBase {
	...
	public void testPostCreation() throws Exception {
		...
		try {
			...
			pf.getProfilePageInstance(ob).clickOnPostModalCloseButton();

			try {
				pf.getProfilePageInstance(ob).validateCancelModalControls(test);
				test.log(LogStatus.PASS, "Cancel modal is displayed up on clicking X button in post modal");
			} catch (Throwable t) {
				test.log(LogStatus.FAIL, "Cancel modal is not displayed up on clicking X button in post modal");
