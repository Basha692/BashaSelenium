...
public class Customercare012 extends TestBase {
	...
	public void testcaseIPA1() throws Exception {
		...
		try {
			...
			try {
				...
				test.log(LogStatus.PASS, "IPA Customer care page displays Support Request sections.");
			} catch (Throwable t) {
				test.log(LogStatus.FAIL, "IPAA Customer care page displays Support Request sections.");
				closeBrowser();
			}
			WebElement IPA_Header = pf.getBrowserActionInstance(ob)
					.getElement(OnePObjectMap.IPA_CC_HEADER_CSS);
			String Header = IPA_Header.getText();
			try {
				if (IPA_Header.isDisplayed())
					Assert.assertEquals(Header, "IP ANALYTICS");
				test.log(LogStatus.PASS, "IPA Customer care page displays IP Analytics header");
			} catch (Throwable t) {
				test.log(LogStatus.FAIL, "IPA Customer care page doesn't  display IP Analytics header");
				closeBrowser();
			}
			closeBrowser();
		} catch (Throwable t) {
