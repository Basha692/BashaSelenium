...
/**
 * Class for Validate HCR Badge in Profile page
 * 
 * @author UC202376
 *
 */
public class Profile41 extends TestBase {
	...
	public void getOtherProfileDetails(String hcrProfile) throws Exception {
		try {
			...
			test.log(LogStatus.FAIL, "HCR Badge not displayed in Search results page");
			...
			test.log(LogStatus.FAIL, "Snapshot below: " + test
					.addScreenCapture(captureScreenshot(this.getClass().getSimpleName() + "_HCR_Profile_badge_fail")));
			closeBrowser();
		}
	}
	 ...
	 * @throws Exception
	 */
	@Test(dependsOnMethods = "getOtherProfileDetails")
	@Parameters("hcrProfile")
	public void hcrBadgeInProfilePage(String hcrProfile) throws Exception {
		try {
			test.log(LogStatus.INFO, "Click HCR profile");
			pf.getProfilePageInstance(ob).clickProfile();
			test.log(LogStatus.INFO, "Validate HCR Badge in Profile page");
			pf.getProfilePageInstance(ob).hcrBadgeValidation(hcrProfile);
			pf.getLoginTRInstance(ob).logOutApp();
			test.log(LogStatus.INFO, this.getClass().getSimpleName() + " execution ends ");
			closeBrowser();
		} catch (Throwable t) {
			test.log(LogStatus.FAIL, "HCR Badge not displayed in Profile page");
			StringWriter errors = new StringWriter();
			t.printStackTrace(new PrintWriter(errors));
			test.log(LogStatus.FAIL, errors.toString());
			ErrorUtil.addVerificationFailure(t);
