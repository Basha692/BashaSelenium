...
public class Profile34 extends TestBase {
	...
	public void FollowingTabScrollValidation() throws Exception {
		try {
			test.log(LogStatus.INFO, "go to user profile page");
			pf.getHFPageInstance(ob).clickProfileImage();
			pf.getProfilePageInstance(ob).clickProfileLink();
			test.log(LogStatus.INFO, "Followers tab records/record count should increase while do scrolldown");
			pf.getProfilePageInstance(ob).followingTabScroll();
			...
			status = 2;// excel
			test.log(LogStatus.INFO, "Snapshot below: " + test
					.addScreenCapture(captureScreenshot(this.getClass().getSimpleName() + "_following_tab_scroll")));
			closeBrowser();
		}
	}
