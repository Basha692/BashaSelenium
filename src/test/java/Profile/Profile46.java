...
public class Profile46 extends TestBase {
	...
	public void validateProifleCTACountryField() throws Exception {
		try {
			test.log(LogStatus.INFO, "Click on profile image and go to profile page");
			pf.getHFPageInstance(ob).clickProfileImage();
			pf.getProfilePageInstance(ob).clickProfileLink();
			test.log(LogStatus.INFO, "Validate CTA in a white box is getting  displayed in country field");
			pf.getProfilePageInstance(ob).validateProfileCTACountryField();
			pf.getLoginTRInstance(ob).logOutApp();
			closeBrowser();

		} catch (Throwable t) {
			test.log(LogStatus.FAIL, "CTA-country field failed");
			status = 2;// excel
			// print full stack trace
			StringWriter errors = new StringWriter();
			t.printStackTrace(new PrintWriter(errors));
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test
					.addScreenCapture(captureScreenshot(this.getClass().getSimpleName() + "CTA-country field failed")));// screenshot
			closeBrowser();
		}
	}
