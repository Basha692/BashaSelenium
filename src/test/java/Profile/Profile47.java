...
public class Profile47 extends TestBase {
	...
	/**
	 * Method for validate profile CTA whitebox summary field
	 * 
	 * @throws Exception, When Profile CTA whitebox summary field not displayed
	 */
	@Test(dependsOnMethods = "testLoginTRAccount")
	public void validateProifleCTASummaryField() throws Exception {
		try {
			test.log(LogStatus.INFO, "Click on profile image and go to profile page");
			pf.getHFPageInstance(ob).clickProfileImage();
			pf.getProfilePageInstance(ob).clickProfileLink();
			test.log(LogStatus.INFO, "Validate CTA in a white box is getting  displayed in summary field");
			pf.getProfilePageInstance(ob).validateProfileCTASummaryField();
			pf.getLoginTRInstance(ob).logOutApp();
			closeBrowser();

		} catch (Throwable t) {
			test.log(LogStatus.FAIL, "CTA-summary field failed");
			status = 2;// excel
			// print full stack trace
			StringWriter errors = new StringWriter();
			t.printStackTrace(new PrintWriter(errors));
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test
					.addScreenCapture(captureScreenshot(this.getClass().getSimpleName() + "CTA-summary field failed")));// screenshot
			closeBrowser();
		}
	}
