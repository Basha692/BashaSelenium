...
public class Profile45 extends TestBase {
	...
	public void validateProiflePicCancelButton() throws Exception {
		try {
			test.log(LogStatus.INFO, "Click on profile image and go to profile page");
			pf.getHFPageInstance(ob).clickProfileImage();
			pf.getProfilePageInstance(ob).clickProfileLink();
			test.log(LogStatus.INFO, "Validate proifle pic modal window cancel status");
			pf.getProfilePageInstance(ob).validateProfilePicUpdateButtonStatus();
			pf.getProfilePageInstance(ob).cancelProfilePicModalWindow();
			pf.getLoginTRInstance(ob).logOutApp();
			closeBrowser();

		} catch (Throwable t) {
			test.log(LogStatus.FAIL, "Proifle pic window should be cancelled");
			status = 2;// excel
			// print full stack trace
			StringWriter errors = new StringWriter();
			t.printStackTrace(new PrintWriter(errors));
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
					captureScreenshot(this.getClass().getSimpleName() + "_profile_pic_window_cancel_fail")));// screenshot
			closeBrowser();
		}
	}
