...
public class Profile66 extends TestBase {
	...
	@Parameters({"enwProfileFlyout", "link"})
	public void validateENWToNeonUsingTermsofUseLink(String enwProfileFlyout,
			String link) throws Exception {
		try {
			test.log(LogStatus.INFO, "Navigate to Neon application from EndNoteWeb using Terms of use link");
			...
			test.log(LogStatus.FAIL, "ENW to Neon navigation using Terms of use link not done");
			status = 2;// excel
			// print full stack trace
			StringWriter errors = new StringWriter();
			t.printStackTrace(new PrintWriter(errors));
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(
					captureScreenshot(this.getClass().getSimpleName() + "ENW_To_Neon_Navigation_Not_Done")));// screenshot
			closeBrowser();
		}
	}
