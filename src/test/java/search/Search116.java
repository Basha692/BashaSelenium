...
public class Search116 extends TestBase {
	...
	public void testcaseB116() throws Exception {
		...
		try {
			...
			int postResultsBeforeScroll = ob
					.findElements(By.cssSelector(OR.getProperty("tr_search_results_post_title_css"))).size();
			((JavascriptExecutor) ob).executeScript("javascript:window.scrollBy(0,document.body.scrollHeight-150)");
			waitForAjax(ob);
			waitForElementTobeClickable(ob, By.cssSelector(OR.getProperty("tr_search_results_post_title_css")), 120);
			waitForElementTobeClickable(ob, By.cssSelector(OR.getProperty("tr_search_results_sortby_button_css")), 120);

			int postResultsAfterScroll = ob
					.findElements(By.cssSelector(OR.getProperty("tr_search_results_post_title_css"))).size();
			...
			test.log(LogStatus.INFO, "Snapshot below: " + test
					.addScreenCapture(captureScreenshot(this.getClass().getSimpleName() + "_post_scroll_failed")));// screenshot
			closeBrowser();
		}
