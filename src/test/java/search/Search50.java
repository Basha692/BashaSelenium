...
public class Search50 extends TestBase {
		...
		try {
			...
			if (!StringContains(text, "Articles")) {
				...
				test.log(LogStatus.FAIL,
						"ARTICLES option not selected in the left navigation pane by default when user searches using ARTICLES option in the search drop down");// extent
																																								// report
				status = 2;// excel
				test.log(LogStatus.INFO, "Snapshot below: " + test.addScreenCapture(captureScreenshot(this.getClass()
						.getSimpleName()
						+ "_ARTICLES_option_not_selected_in_the_left_navigation_pane_by_default_when_user_searches_using_ARTICLES_option_in_the_search_drop_down")));// screenshot
