 ...
 *
 */
public class IPA08 extends TestBase {
	...
	public void testLoginIPA(String username, String password) throws Exception {
		...
		try {
			...
			if(defaultvalue.equalsIgnoreCase("Date added"))
				test.log(LogStatus.PASS, "Date added is the default option selected in sort Dropdown");
			else
				{	test.log(LogStatus.FAIL, "Date added is not the default option selected in sort Dropdown");
			...
			*/
			...
			pf.getDashboardPage(ob).sortByText("Date added");
			dates=pf.getDashboardPage(ob).getDateList(OnePObjectMap.NEON_IPA_RESULTLIST_PUBLICATION_DATES_CSS);
			datessorted =new ArrayList<>(dates);
			Collections.sort(datessorted,Collections.reverseOrder());
			if(dates.equals(datessorted))
				test.log(LogStatus.PASS, "Sort By Date added  is working as Expected when selecting after some other sorts");
			else
