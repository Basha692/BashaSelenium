...
public class IPAIAM0006 extends TestBase {
	...
	public void testcaseh2() throws Exception {
		...
		try {
			...
			pf.getLoginTRInstance(ob).loginWithFBCredentials(LOGIN.getProperty("IPAfbuser6"),
					LOGIN.getProperty("IPAfbpw6"));
			test.log(LogStatus.PASS, "user has logged in with social account");
			pf.getIpaPage(ob).clickIPALink();
			test.log(LogStatus.PASS, "user is navigate to Step up Auth modal");
			pf.getIpaPage(ob).validateIPAStepUPModalTitle(test);
			pf.getIpaPage(ob).validateFirstTextOnIPAStepUp(test);
			test.log(LogStatus.INFO, "Verified whether user is able to see First text on step up modal");
			pf.getIpaPage(ob).validateSecondTextOnIPAStepUP(test);
